...
//-- 强制开启 zoom：以下方法通过 _vehicle 发送真实 MAVLink 命令。
//-- 目标组件使用飞机默认组件 ID（SimulatedCameraControl 没有真实相机组件 ID）。
void SimulatedCameraControl::setZoomLevel(qreal level)
{
    qCDebug(SimulatedCameraControlLog) << "Set zoom level" << level;

    //-- Limit
    level = qBound<qreal>(0.0, level, 100.0);
...
void SimulatedCameraControl::stepZoom(int direction)
{
    qCDebug(SimulatedCameraControlLog) << "Step zoom" << direction;
...
void SimulatedCameraControl::startZoom(int direction)
{
    qCDebug(SimulatedCameraControlLog) << "Start zoom" << direction;
...
{
    qCDebug(SimulatedCameraControlLog) << "Stop zoom";
